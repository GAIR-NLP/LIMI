...
class GLM4MoEBridge(Qwen2MoEBridge):
    ...
    _MLP_MAPPING = {
        ...
        **(Qwen2Bridge._MLP_MAPPING),
        ...
        # Special parameters for the last MoE layer (layer 92)
        ...
        "shared_head.head.weight": ["model.layers.{layer_number}.shared_head.head.weight"],
        "shared_head.norm.weight": ["model.layers.{layer_number}.shared_head.norm.weight"],
        "embed_tokens.weight": ["model.layers.{layer_number}.embed_tokens.weight"],
        "layer_embed_tokens.weight": ["model.layers.{layer_number}.embed_tokens.weight"],
    }
    ...
    def _weight_name_mapping_hf_to_mcore(self, hf_weights_name: str) -> list[str]:
        """
        Map Hugging Face weight names to MCore weight names.
        
        Args:
            hf_weights_name: Hugging Face weight name
            
        Returns:
            list: Corresponding MCore weight names
        """
        # Handle special parameters for GLM4.5 model
        # Check for specific patterns like model.layers.92.embed_tokens.weight
        layer_match = re.search(r"model\.layers\.(\d+)\.", hf_weights_name)
        if layer_match:
            layer_number = layer_match.group(1)
            
            # Map special parameters
            if ".embed_tokens.weight" in hf_weights_name:
                mcore_name = f"module.module.decoder.layers.{layer_number}.layer_embed_tokens.weight"
                print(f"Mapping HF to MCore: {hf_weights_name} -> {mcore_name}")
                return [mcore_name]
            elif ".eh_proj.weight" in hf_weights_name:
                mcore_name = f"module.module.decoder.layers.{layer_number}.eh_proj.weight"
                print(f"Mapping HF to MCore: {hf_weights_name} -> {mcore_name}")
                return [mcore_name]
            elif ".enorm.weight" in hf_weights_name:
                mcore_name = f"module.module.decoder.layers.{layer_number}.enorm.weight"
                print(f"Mapping HF to MCore: {hf_weights_name} -> {mcore_name}")
                return [mcore_name]
            elif ".hnorm.weight" in hf_weights_name:
                mcore_name = f"module.module.decoder.layers.{layer_number}.hnorm.weight"
                print(f"Mapping HF to MCore: {hf_weights_name} -> {mcore_name}")
                return [mcore_name]
            elif ".shared_head.head.weight" in hf_weights_name:
                mcore_name = f"module.module.decoder.layers.{layer_number}.shared_head.head.weight"
                print(f"Mapping HF to MCore: {hf_weights_name} -> {mcore_name}")
                return [mcore_name]
            elif ".shared_head.norm.weight" in hf_weights_name:
                mcore_name = f"module.module.decoder.layers.{layer_number}.shared_head.norm.weight"
                print(f"Mapping HF to MCore: {hf_weights_name} -> {mcore_name}")
                return [mcore_name]
        
        # Fall back to parent class implementation
        return super()._weight_name_mapping_hf_to_mcore(hf_weights_name)
    ...
    def _weight_name_mapping_mcore_to_hf(self, mcore_weights_name: str) -> list[str]:
        ...
        """
        print("mcore_weights_name:",mcore_weights_name)
        assert (
        ...
        if "mtp" in mcore_weights_name:  # first check mtp
            ...
            # Check for special parameters that might not be in mlp/attention/mtp
            for keyword, mapping_names in self._MLP_MAPPING.items():
                if keyword in mcore_weights_name:
                    # Extract layer number from the parameter name
                    layer_match = re.search(r"decoder\.layers\.(\d+)", mcore_weights_name)
                    if layer_match:
                        layer_number = layer_match.group(1)
                        print(f"Mapping special parameter: {mcore_weights_name} -> {[x.format(layer_number=layer_number) for x in mapping_names]}")
