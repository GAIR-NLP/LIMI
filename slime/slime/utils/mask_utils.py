...
class MultiTurnLossMaskGenerator:
    ...
    def gen_multi_turn_loss_mask_qwen(self, messages: List[Dict], tools: List[Dict]) -> Tuple[List[int], List[int]]:
        all_loss_masks = []
        all_token_ids = []

        for i, message in enumerate(messages):
            if i==0 and tools:
                message_ids = self.tokenizer.apply_chat_template([message], tools, tokenize=True)
            else:
                message_ids = self.tokenizer.apply_chat_template([message], tokenize=True)[2:] #去掉special token
    ...
    def get_loss_mask(self, messages: List[Dict], tools: List[Dict]) -> List[int]:
